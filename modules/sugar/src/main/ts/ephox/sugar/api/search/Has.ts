...
const ancestor = function (element: Element<DomNode>, target: Element<DomNode>) {
  return PredicateExists.ancestor(element, Fun.curry2(Compare.eq, target));
};
...
const sibling = function (element: Element<DomNode>, targets: Element<DomNode>[]) {
  return PredicateExists.sibling(element, (elem) => Arr.exists(targets, Fun.curry2(Compare.eq, elem)));
};

const child = function (element: Element<DomNode>, target: Element<DomNode>) {
  return PredicateExists.child(element, Fun.curry2(Compare.eq, target));
};

const descendant = function (element: Element<DomNode>, target: Element<DomNode>) {
  return PredicateExists.descendant(element, Fun.curry2(Compare.eq, target));
};
